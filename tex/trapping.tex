...
\blue{We comment that these solutions make use of the shearing box
  equations, and are thus subject to the same limitatations as that
...
where $z\equiv ka$ \blue{(not to be confused with the Cartesian co-ordinate)}, and
...
\blue{\subsection{Dust - gas constrast}}
...
and $\rho_g$. These, in turn, need the dependencies on $z$ (the
vertical coordinate, not $ka$ as used in
\sect{sect:nonaxisymmetric}). These are straightforward, being $\exp(-z^2/2H^2)$ for the gas and
