...
inside the vortex core.  \blue{Based on our solution, we derive quantities such as the gas-dust constrast, the trapped dust mass,
and the dust constrast at the same orbital location. We apply our model to the recently imaged Oph IRS 48 system, finding 
...
range of relevance, they both yield values around $H/2$, which matches
the results of numerical simulations.}

\blue{Substituting \eq{eq:sonic} in \eq{eq:gas_axi}, the gas density contrast is 
\beq
  \frac{\rho_{g\,{\rm max}}}{\rho_{g\,{\rm min}}} =
  \exp{\left[\frac{f^2(\chi)}{2\chi^2\omega_V^2}\right]}, \label{eq:gas-contrast}
\eeq
\noindent For neither the Kida not the GNG solutions does this quantity
deviate much from unity. The argument in the exponent tends
...
parameters derived by \cite{vanderMarel13}.The dust constrast in the same orbit is 130, which, according to
\eq{eq:max-contrast} and \eq{eq:gas-contrast} for $\chi=3.1$, set $S=4.79$ and
$S=4.82$ for the Kida and GNG solutions, respectively. } 

\blue{The dust temperature is estimated at 60\,K. Assuming this is the
  same as the gas temperature, and that the gas is a 5:2 hydrogen to
  helium mixture (mean molecular weight of 2.4), the isothermal sound speed is 
...
take the mean (in log), $\approx 10^{-14}$ g\,cm$^{\rm  -3}$, as representative. For particles of material density 0.8 g\,cm$^{\rm  -3}$, the
Stokes number should be $\St \approx 0.25$. That yields
$\delta \approx 5\times 10^{-2}$, meaning typical turbulent velocities in the
