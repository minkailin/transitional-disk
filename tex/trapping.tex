...
\blue{If a direct determination of $\delta$ does not sound promissing,
  an indirect way is possible by measuring $S$ and $\St$. The
  parameter $S$ can be determined via the dust-density
  constrast with \eqs{eq:max-contrast}{eq:gas-contrast}, or via the
  dust-gas constrast at maximum \eqp{eq:maxrhod}. The Stokes number is 
\beq
...
close because the gas constrast is small \eqp{eq:gas-contrast}.}
