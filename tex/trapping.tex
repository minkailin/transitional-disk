...
%$Y_0(z)$ is the zeroth Bessel function of second kind. In terms of
%Bessel functions with real arguments, we recall that 

%\beqn
%J_n(iz) &=& i^n I_n(z) \\
%I_n(z) &=& i^{-n}\left[ {\rm csc}(\pi n) Y_{-n}(iz) - {\rm cot} (\pi n) Y_n(iz)\right]
%\eeqn

%\noindent for $n=0$, the second type Bessel funtions are equal, so the
%term in parentheses 

%the term in brackets becomes $I_0(z)$, and 

%Y_n(iz) &=& i^{(n+1)} I_n(z) + \frac{2}{\pi} i^n K_n(z)
%\eeqn where z is a complex argument, and I and K are the modified
%Bessel functions of first and second kind, respectively. The solution
%then becomes

%\beq
%y(a) = c_1 I_0 (ka) + c_2 i I_0(-ka) + \frac{2c_2}{\pi} K_0(-ka) 
%\eeq
