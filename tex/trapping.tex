...
\blue{The constrast in the same orbit is found by calculating the
...
\beq
\frac{\rho_{d\,{\rm max}}}{\rho_{d\,{\rm min}}} = \frac{\rho_{g\,{\rm max}}}{\rho_{g\,{\rm min}}} \exp{(S)},\
\eeq
...
%
 ...
 writing the vortex velocities \eqp{eq:vortex} as Mach numbers 
\beq
  {\rm Ma}_x = \frac{u_x}{c_s} = \omega_V\chi^{-1} \  y/H \qquad  {\rm Ma}_y = \frac{u_y}{c_s}= -\omega_V\chi \ x/H,
  \label{eq:vortex-mach}
\eeq and setting ${\rm Ma}_x = {\rm Ma}_y - u_K/c_s = 1$, where
$u_K=q\varOmega  x$ is the linearized Keplerian velocity. This yields the
boundary at 
\beq
a_s = H/\omega_v
\eeq where the subscript $s$ stands for
sonic. Substituting that in \eq{eq:gas_axi}, the gas density contrast is 
\beq
  \frac{\rho_{g\,{\rm max}}}{\rho_{g\,{\rm min}}} = \exp{\left[\frac{f^2(\chi)}{2\omega_V^2}\right]},
\eeq
\noindent which translates into a contrast of $\approx$2.16 for a Kida
vortex, and $\approx$1.56 for a GNG vortex. These values, times
$\exp{(S)}$, yield the dust contrast.} 
...
lines. The turbulence is subsonic, at about 10\% of the sound speed \citep{Lesur-Papaloizou10,Lyra-Klahr11}. 
...
parameters derived by \cite{vanderMarel13}. The dust constrast in the
same orbit is 130, which sets $S\approx 2.2$ for a Kida vortex, or
$S\approx 3.1$ for a GNG vortex. The gas ratio would allow to
determine the vortex model but, unfortunately, for that system, the gas
density at the location of the maximum is blocked by interstellar
absorption.} 

\blue{The dust temperature is estimated at 60\,K, which for a $2
  M_\sun$ star translates into $H/r \approx 0.06$ for the
disk aspect ratio. For their vortex located at 63\,AU, $H \approx
4$\,AU. The particle radius measurement is $a_\bullet \geq 4$\,mm. The 
gas density is estimated \citep{Brown12} at $\rho_g \approx 5\times 10^{-14}$ g\,cm$^{\rm
  -3}$. For particles of material density 0.8 g\,cm$^{\rm  -3}$, the
Stokes number should be $\St \gtrsim 0.07$. That yields
$\delta \gtrsim 0.02$, meaning typical turbulent velocities in the
vortex core at $\sqrt{\delta} \approx 15\%$ of the sound speed. Though
slightly on the high side, they are consistent with the velocities
expected for the elliptic instability \citep{Lesur-Papaloizou10,Lyra-Klahr11}.}

\blue{The aspect ratio is $\chi=3.1$. For the typical interstellar
dust-to-gas ratio of $\varepsilon=0.01$, the trapped mass yields
$\approx 2$ Earth masses. Given the uncertainties, it is in the
correct order of magnitude as measured (9 Earth masses).}
