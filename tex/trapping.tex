...
semi-minor axis $a$ and semimajor axis $a\chi$). \blue{Notice that the flow eventually
  ...
  validity of the solution, as the vortices shock.} In this work
...
\blue{We comment that the GNG solution was used to model vortices
  found in non-linear hydrodynamic simluations of the
  Papaloizou-Pringle instability \citep{Hawley87}, which is similar to
  the RWI. Recently, \cite{Lin-Papaloizou11a} explicitly simulated the
  RWI at planetary gap edges and found that in quasi-steady state, the
  RWI vortices resemble  vortices formed by perturbing the disk with
  the Kida solution. We thus believe the above solutions are suitable
...
\section{``Axisymmetric'' \red{dust distribution} \blue{solution}}
\label{sect:axisymmetric}

\blue{\subsection{Midplane dust distribution}}
...
\blue{\subsection{Midplane gas distribution}} % and dust density enhancement}}
...
\blue{\subsection{Vertical dependencies and dust-gas constrast}}
...
and $\rho_g$. These, in turn, need the dependencies on $z$, that we
calculate in the next subsection}
