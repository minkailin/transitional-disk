...
We now consider the non-axisymmetric problem ($\partial_\phi\neq0$) 
...
$\partial_x\rho_d=\partial_y\rho_d=0$ at the origin. That is, dust
reaches maximal density there.  
...
\subsection{$z\ll 1$}
As $a\to 0$, We expect this to be valid as $a\to0$ because the non-axisymmetric 
components vanish at the origin, so $\rho_0$ becomes the dominant term
in \eq{eq:series}. Furthermore, we require $|\rho_m|$ to decrease with
respect to $m$ in order to achieve convergence.

In this section we seek the simplest non-axisymmetric solution 
\beq
