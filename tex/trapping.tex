...
%Strom et al. 1989 : identification
...
vortex flow alone. Note that we are effectively considering regions
close to the vortex center because $\nabla h$ vanishes there, while
...
$\v{v}=\v{u}+\tau\grad{h}$ case and, (dashed line) the $\v{v}=\v{u}$ case.
The exponential and Bessel solutions agree up to 10\% up to $x$=1.5, and up 
to 1\% up to $x$=1. We conclude that this approximate solution is quite accurate 
...
dust-trapping equation is then comparing advection due to the
background vortex flow and diffusion. Physically, we expect that
